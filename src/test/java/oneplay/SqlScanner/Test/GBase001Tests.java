...
public class GBase001Tests {
    ...
    private String path = "D:\\IdeaProject\\SqlScanner\\examples\\001多表关联聚合时优先聚合计算.sql";
    ...
    public void test001() throws Exception {
        ...
        GBase001 GBase001 = new GBase001();
        logger.info("Matching ...");
        if (!GBase001.match(parseTree).isEmpty()) {
            GBase001.printRuleInfo();
        }
    }

    @Test
    public void test001mysql() throws Exception {
        Configurator.setAllLevels("oneplay.SqlScanner", Level.DEBUG);
        ParseTree parseTree = ParseTreeUtils.getParseTree("MySql", new FileInputStream(new File(path)));
        BaseRule rule = new MySql001();
        logger.info("Matching ...");
